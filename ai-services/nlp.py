...
# Urgency keywords mapping (kept local — no API needed for this)
URGENCY_KEYWORDS = {
...
def summarize_text(text, max_length=50, min_length=20):
    ...
    try:
        prompt = f"""Summarize the following civic complaint in one clear, concise sentence (max 50 words).
...
Reply with just the summary sentence, nothing else."""
        ...
        # Remove quotes if Gemini wraps in them
...
def generate_description(category):
    """
    Generate a detailed civic description for a detected issue category using Gemini.
...
def analyze_text_comprehensive(text):
    ...
    Uses a single Gemini call for efficiency.
...
Nothing else."""
        ...
        # Urgency is always local (no API needed)
        ...
        # Fallback to keyword-only urgency
        urgency_level, urgency_label, keywords_found = detect_urgency(text)
        return {
            "classification": {"category": "Uncategorized", "confidence": 0.0},
            "summary": text[:100],
